...
2. remove_all   (clear)
*/
bool find_val_arr(struct Arr* pArr, int val);
bool remove_all(struct Arr* pArr);
...
int main(void)
{
    ...
    show_arr(&arr);                                     // 10 2 33 24 6
    
    reverse_arr(&arr);
    show_arr(&arr);                                     // 6 24 33 2 10
    
    sort_arr1(&arr);
    show_arr(&arr);                                     // 2 6 10 24 33
    
    sort_arr2(&arr);
    show_arr(&arr);                                     // 33 24 10 6 2
    
    find_val_arr(&arr, 10);                             // Found it, the index of 10: 2
    find_val_arr(&arr, 11);                             // Sorry, not found 11
    
    remove_all(&arr);                                   // This array is clear, now.
    show_arr(&arr);                                     // Array is empty!
...
bool find_val_arr(struct Arr* pArr, int val)
{
    ...
    else
    {
        for(int i=0; i<pArr->cnt; ++i)
        {
            if(val == pArr->pBase[i])
            {
                printf("Found it, the index of %d: %d\n", val, i);
                flag = 1;
                break;
            }
        }
...
bool remove_all(struct Arr* pArr)
{
    if(is_empty(pArr))
    {
        printf("This array is already clear.\n");
        return false;
    }
    else
    {
        pArr->cnt = 0;
        printf("This array is clear, now.\n");
        return true;
    }
}
